...
let real_speed = computed(()=> miner_started.value ? Number((speed.value * (data.value.indexOf(miner_power.value) + .8)).toFixed(2)) : 0)
...
let radialBar_series = computed(() => [real_speed.value / (speed_max.value / 100)])
            ...
            <div class="row miner">
                <div class="col-12 col-sm-5 order-2 order-sm-1 d-flex d-sm-block flex-wrap">
                    <div class="miner_statistics">
                        ...
                        <div class="miner_statistics_val"><span>{{ state.xmr.day }}</span> XMR / {{ $t("main.day") }}</div>
                        <div class="miner_statistics_val small">= {{ state.usd.day }} USD / {{ $t("main.day") }}</div>
                    </div>
                    <div class="miner_statistics">
                        <div class="label">{{ $t("main.miner_statistics_label_session") }}</div>
                        <div class="miner_statistics_val blue"><span>{{ state.xmr.session }}</span> XMR</div>
                        <div class="miner_statistics_val small">= {{ state.usd.session }} USD</div>
                    </div>
                    <div class="miner_statistics">
                        <div class="label">{{ $t("main.miner_statistics_label_time") }}</div>
                        <div class="miner_statistics_val blue"><span>{{ state.xmr.time }}</span> XMR</div>
                        <div class="miner_statistics_val small">= {{ state.usd.time }} USD</div>
                    </div>
                </div>
                    ...
                    <div class="miner_speed_outer">
                        <div class="miner_speed_area">
                            ...
                            <div class="miner_speed_filled">
                                <apexchart height="360px" type="radialBar" :options="radialBar_options"
                                    :series="radialBar_series" />
                            </div>
                            <div class="miner_speed_dashed">
                                <apexchart height="260px" type="radialBar" :options="radialBar_options2"
                                    :series="radialBar_series" />
                            </div>
                ...
                <template #content>
                    <div class="row miner_main">
                        ...
                        <div class="col-12 col-md-8 order-1 order-md-2">
                            <div :class="[{ active: miner_started }, 'miner_balance']">{{ wallet.monero }} <app-icon
                                    name="monero" size="28" /></div>
                            <div class="miner_balance_info">{{ $t("miner.approx") }}: <span>{{ wallet.usd }} USD</span>
                            </div>
                    ...
                    <div class="row miner_power">
                        <div class="col-12 col-md-4"><Label :label='$t("main.miner_power")' icon="power" /></div>
                        <div class="col-12 col-md-8">
                            <vue-slider v-model="miner_power" :marks="true" :data="data" :included="true" :absorb="true"
                                :dotSize="24" :height="10" :lazy="true" tooltip="none"></vue-slider>
                        </div>
                    </div>
                </template>
            </Panel>
...
.miner_button {
    ...
    &.filled {
        background: linear-gradient(93.49deg, #01CAC4 -19.41%, #0278F6 116.58%), #D9D9D9;
